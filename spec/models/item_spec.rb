...
RSpec.describe Item, type: :model do
  ...
  describe 'class methods' do
    ...
    it 'can return an item by name search' do
      create :item, name: 'spatula'
      item2 = create :item, name: 'spoon'
      create :item, name: 'fork'
      item4 = create :item, name: 'knife'

      expect(Item.find_using_name('kn')).to eq(item4)
      expect(Item.find_using_name('spo')).to eq(item2)
    end
    it 'can return an item by by max and min search' do
      item1 = create :item, name: 'spatula', unit_price: 15.00
      create :item, name: 'spoon', unit_price: 20.00
      item3 = create :item, name: 'fork', unit_price: 10.00
      create :item, name: 'knife', unit_price: 12.00

      expect(Item.find_using_price(10, 15)).to eq(item3)

      expect(Item.find_using_price_max(10)).to eq(item3)
      expect(Item.find_using_price_min(15)).to eq(item1)
    end
  end
end
