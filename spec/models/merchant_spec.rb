...
RSpec.describe Merchant, type: :model do
  ...
  describe 'Class Methods' do
    ...
    it 'can return a list of merchants by name search' do
      ...
      expect(Merchant.find_all_using_name('Steves')).to eq([merchant1])
      expect(Merchant.find_all_using_name('es')).to eq([merchant3, merchant2, merchant1])
      expect(Merchant.find_all_using_name('stuff')).to eq([merchant4])
    end
  end
